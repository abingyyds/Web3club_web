...
function setupManageButtons() {
    ...
    manageButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            const clubDomain = btn.closest('.club-item').querySelector('.club-domain').textContent;
            openSettingsModal(clubDomain);
        });
    });
}
...
function setupAccessButtons() {
    ...
    accessButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            // 获取俱乐部域名
            const clubItem = btn.closest('.club-item');
            let clubDomain;
            
            // 检查是否是已注册的俱乐部还是已加入的俱乐部
            const domainElement = clubItem.querySelector('.club-domain');
            const nameElement = clubItem.querySelector('.club-name');
            
            if (domainElement) {
                clubDomain = domainElement.textContent; // 已经包含 .web3.club
            } else if (nameElement) {
                clubDomain = `${nameElement.textContent}.web3.club`;
            }
            
            // 构建完整的 URL 并跳转
            const url = `https://${clubDomain}`;
...
// Open settings modal
function openSettingsModal(clubDomain) {
    const modal = document.querySelector('.settings-modal');
    const modalTitle = modal.querySelector('h3');
    
    // Update modal title with club domain
    modalTitle.textContent = `Settings - ${clubDomain}`;
...
// Show/hide template gallery based on radio selection
templateDisplay.addEventListener('change', function() {
    templateGallery.style.display = this.checked ? 'block' : 'none';
    if (this.checked && !templatesGrid.children.length) {
        loadTemplates();
    }
});
...
const availableTemplates = [
    ...
    {
        name: 'Vibrant Template',
        filename: 'vibrant.png'
    }
];
...
function createTemplateItem(template) {
    ...
    img.src = `assets/CLUBTemplate/${template.filename}`;
    ...
    // Click to preview
    item.addEventListener('click', () => showPreview(template));
    
    // Click to select
    item.addEventListener('click', (e) => {
        if (!e.target.closest('.template-preview-modal')) {
